...
// `host` is the supplier's realtime host: fal.run, or Decart's signaling host.
async function checkNetwork({ resolveProxy = null, platform = process.platform, windowsAdapters = defaultWindowsAdapters, host = REALTIME_HOST } = {}) {
  let ip = null;
  let dnsError = null;
  try {
  ...
  const [connects, gathered, proxy] = await Promise.all([
    ...
    resolveProxy ? Promise.resolve(resolveProxy(`https://${host}`)).catch(() => null) : null
  ]);
